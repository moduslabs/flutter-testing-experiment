...
main() {
  group('TaskService', () {
    ...
    setUp(() async {
      ...
      await Future.wait(
          tasks.map((task) => instance.collection('tasks').add(task.toJson())));
      ...
      getIt.registerLazySingleton<TaskService>(
          () => TaskService(getIt<TaskCollectionReference>()));
    });

    tearDown(() async {
      final tasks = await instance.collection('tasks').get();
      await Future.wait(
          tasks.docs.map((element) => element.reference.delete()));
    ...
    test('Should get all tasks', () async {
      ...
      expect(tasks.map((task) => task.title),
          orderedEquals(['Task 1', 'Task 2', 'Task 3']));
    });
    ...
    test('Should get pending tasks', () async {
      ...
      expect(
          tasks.map((task) => task.title), orderedEquals(['Task 1', 'Task 3']));
    });
    ...
    test('Should remove a given task with its id', () async {
      ...
      final queryAfterRemoving = await instance
          .collection('tasks')
          .where('title', isEqualTo: 'Task 1')
          .get();
    ...
    test('Should save a new task and return its id', () async {
      final taskService = getIt<TaskService>();
      final task = await taskService.save(Task(
          title: 'New Task', dueDate: DateTime.now(), description: 'New Task'));
      expect(task.id, isNotNull);
      final query = await instance
          .collection('tasks')
          .where('title', isEqualTo: 'New Task')
          .get();
      expect(query.docs.isEmpty, isFalse);
    });
    ...
    test('Should update a new task and return its id', () async {
      ...
      final queryAfterRemoving = await instance
          .collection('tasks')
          .where('title', isEqualTo: 'Updated Task')
          .get();
